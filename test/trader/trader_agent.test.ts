...
describe
  ...
  .concurrent("trader agent commission tests", () => {
    ...
    const PROVIDER_RATE = 0.05; // 5% trader commission
    const AGENT_RATE = 0.02; // 2% agent commission
    const MERCHANT_NET_RUB = AMOUNT_RUB * (1 - SELF_RATE); // 900 RUB

    test.concurrent(
      "approved payin with trader agent merchant commission",
      ({ ctx }) =>
    ...
    test.concurrent(
      "approved payout with trader agent merchant commission",
      ({ ctx }) =>
        ctx.track_bg_rejections(async () => {
          let trader = await ctx.create_random_trader({
            usdt: true,
