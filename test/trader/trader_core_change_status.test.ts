...
describe
  ...
  .concurrent(`trader core manage payin change status`, () => {
    ...
    test.skip("payin approved -> declined", ({ ctx, merchant }) =>
    ...
    test.skip("payin declined -> approved", ({ ctx, merchant }) =>
      ctx.track_bg_rejections(async () => {
        let trader = await setup(ctx, merchant);
...
describe
  ...
  .concurrent("payout cancellation with payout hold period", () => {
    ...
    test.concurrent("approved -> declined after worker (usdt)", ({ ctx, merchant }) =>
      ctx.track_bg_rejections(async () => {
        let trader = await ctx.create_random_trader({
          usdt: true,
          payout_hold_period: 1,
    ...
    test.concurrent("approved -> declined after worker (rub)", ({ ctx, merchant }) =>
      ctx.track_bg_rejections(async () => {
        let trader = await ctx.create_random_trader({
          usdt: false,
          payout_hold_period: 1,
