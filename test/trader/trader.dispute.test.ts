  ...
  describe
    ...
    .concurrent(`trader disputes (${currency})`, () => {
      ...
      test
        ...
        .concurrent(
          "dispute on approved payin draws from deposit wallet",
          ({ ctx }) =>
            ctx.track_bg_rejections(async () => {
              let { trader, merchant } = await setup(ctx);
              ...
              await trader.wallets().then(({ main, income, deposit }) => {
                ...
                assert.containSubset(
                  deposit,
                  { available: DEPOSIT_AMOUNT_MAJOR, held: 0 },
              ...
              assert.deepEqual(
                ...
                {
                  available: AMOUNT_MAJOR + AMOUNT_MAJOR + DEPOSIT_AMOUNT,
                  held: 0,
                },
      ...
      test.concurrent("dispute on declined payin draws from deposit wallet", ({
        ...
        ctx.track_bg_rejections(async () => {
          ...
          await merchant.create_dispute({
            token: res.token,
            amount: (DEPOSIT_AMOUNT + AMOUNT) * STATIC_RATE,
            file_path: assets.PngImgPath,
            description: "test dispute",
          });
