  ...
  test
    ...
    .concurrent(`${name} sms test`, { timeout: 90_000, only }, ({ ctx }) =>
      ctx.track_bg_rejections(async () => {
        let internalSim = [...Array(3)].map(() => crypto.randomBytes(4).toString("hex")).join(".");
        let { bank, trader, merchant, setup } = await setup_trader_with_bank(ctx);
        ...
        for (let sms of sms_messages) {
          let sms_res = await trader.driver.send_sms({
            ...
            from: typeof sms.from === "function" ? sms.from(common.amount) : sms.from,
            text: typeof sms.text === "function" ? sms.text(common.amount) : sms.text,
          ...
          if (sms.parsed_amount || sms.parsed_card || sms.parsed_payer) {
            ...
            if (sms.parsed_card !== undefined) {
              assert.strictEqual(parsed.card, sms.parsed_card, "parsed sms card should match");
            }
            ...
            if (sms.parsed_payer !== undefined) {
              assert.strictEqual(parsed.payer, sms.parsed_payer, "parsed sms payer should match");
            }
          }
        }
        if (should_fail) {
...
describe.runIf(CONFIG.in_project(["a2"])).concurrent("sms parser basics", () => {
    ...
    sms_messages: [
      {
        from: "900",
        parsed_amount: common.amount / 100,
        text: (amount) => `Зачисление ${format_amount(amount)} р Карта *1234 Баланс: 5 000.00 р`,
      },
    ],
    parsers: [
      {
...
describe.runIf(CONFIG.in_project("reactivepay")).todo("payer_pattern tests", () => {
...
describe.runIf(CONFIG.in_project(["reactivepay", "a2"])).concurrent("card_mask parsers", () => {
...
describe.runIf(CONFIG.in_project(["reactivepay"])).todo("INR sms parser (surname mess)", () => {
...
describe.runIf(CONFIG.in_project(["reactivepay"])).concurrent("New implementation tests", () => {
  test_new_bank_sms("happy path", {
    requisite_type: "card",
    sms_messages: [
      {
