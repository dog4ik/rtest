...
test.concurrent(
  ...
  async ({ ctx, merchant, brusnika: _, onTestFailed }) => {
    onTestFailed(async (ctx) => {
      await ctx.annotate("This test can fail if required commission definitions are missing");
    });
    await ctx.track_bg_rejections(async () => {
      await merchant.set_settings(providers("RUB", BrusnikaPayment.settings(ctx.uuid)));
...
test.concurrent("payin insufficient balance default", async ({ ctx, merchant, onTestFailed }) => {
  await ctx.track_bg_rejections(async () => {
    onTestFailed(async (ctx) => {
      await ctx.annotate("This test can fail if required commission definitions are missing");
    });
    ...
    await merchant.create_payment(default_gateway.request("RUB", 1000, "pay", true));
    await notification;
  });
});
...
test.concurrent("refund on unsupported gateway", async ({ ctx, merchant, brusnika }) => {
  await ctx.track_bg_rejections(async () => {
    await merchant.set_settings(providers("RUB", BrusnikaPayment.settings(ctx.uuid)));
...
test.concurrent("refund with no balance for commission", async ({ ctx, merchant }) => {
  await ctx.track_bg_rejections(async () => {
    await merchant.set_settings(default_gateway.fullSettings("RUB"));
    let notification = merchant.queue_notification((notification) => {
      assert.strictEqual(notification.status, "approved");
  ...
  .concurrent(
    ...
    async ({ ctx, merchant }) => {
      await ctx.track_bg_rejections(async () => {
        ...
        await merchant.set_settings(defaultSettings("EUR", RoyalpayPayment.settings(ctx.uuid)));

        ctx.annotate(`MID: ${merchant.id}`);

        let transaction_notification = merchant.queue_notification((notification) => {
          assert.strictEqual(notification.status, "approved");
        });
        royalpay.queue(payment.create_handler()).then(() => payment.send_callback("ok", ctx.uuid));
        ...
        royalpay.queue(payment.create_refund_handler("pending")).then(async () => {
          await delay(5_000);
        ...
        assert.strictEqual(w.held, 0, "held should be empty afetr secusseful refund");
      });
    },
  );
