...
describe
  ...
  .concurrent("pcidss tbank gateway", () => {
    describe.concurrent("tbank card", () => {
      ...
      test.concurrent("tbank approved status", { timeout: 90_000 }, async ({
        ctx,
      }) => {
      ...
      test.concurrent("tbank declined status", { timeout: 90_000 }, async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
      ...
      test.concurrent("tbank card payout pending if known error", async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
      ...
      test.concurrent("tbank card add_customer declined if known error", async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
      ...
      test.concurrent("tbank card init declines if shop is blocked", async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
    ...
    describe.concurrent("tbank sbp", () => {
      ...
      test.concurrent("tbank sbp init declines if invalid params", async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
      ...
      test.concurrent("tbank sbp payout pending if known error", async ({
        ctx,
      }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
      ...
      test.concurrent("tbank sbp payout pending if time out", {
        timeout: 120_000,
      }, async ({ ctx }) => {
        await ctx.track_bg_rejections(async () => {
          let { merchant, tbank, payment } = await setupMerchant(ctx);
