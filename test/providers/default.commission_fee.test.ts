...
describe.concurrent("default provider payin commission with fee: ChangeStateByStatus", () => {
  ...
  test.concurrent("approved → declined (fee + commission reversal)", ({ ctx }) =>
    ctx.track_bg_rejections(async () => {
      ...
      assert.approximately(feed.amount, AMOUNT_RUB, 0.01, "feed: original amount");
      ...
      await delay(2_000);
      await ctx.shared_state().core_harness.change_state_by_status(feed.id, "declined");
      ...
      assert.strictEqual(updated_feed.status, 2, "feed: changed to declined");
      assert.strictEqual(
  ...
  test.concurrent("declined → accepted with fee and amount change", ({ ctx }) =>
    ctx.track_bg_rejections(async () => {
      let merchant = await setup(ctx);
      ...
      await delay(2_000);
      await ctx.shared_state().core_harness.change_state_by_status(feed.id, "accepted", {
      ...
      let updated_feed = await ctx.get_feed(response.token);
      assert.strictEqual(updated_feed.status, 1, "feed: changed to approved");
      assert.approximately(
        updated_feed.amount,
        PAID_AMOUNT_RUB,
        0.01,
...
describe.concurrent("default provider payout commission with fee: ChangeStateByStatus", () => {
  ...
  test.concurrent("declined payout with fee: merchant balance unchanged", ({ ctx }) =>
    ctx.track_bg_rejections(async () => {
      let merchant = await setup(ctx);
      ...
      let feed = await ctx.get_feed(response.token);
      assert.strictEqual(feed.status, 2, "feed: declined");
      assert.approximately(feed.amount, AMOUNT_RUB, 0.01, "feed: amount unchanged");
    }),
  );
});
