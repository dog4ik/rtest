...
describe.concurrent("default provider payin commission with fee: ChangeStateByStatus", () => {
  ...
  const _PROVIDER_COMMISSION_RUB = AMOUNT_RUB * PROVIDER_RATE + PROVIDER_FEE; // 50 + 25 = 75 RUB
  ...
  test.concurrent("approved → declined (fee + commission reversal)", ({
    ctx,
  }) =>
    ctx.track_bg_rejections(async () => {
      ...
      assert.isNotNull(feed.commission_amount, "feed: commission_amount set");
      assert.approximately(
      ...
      assert.isNotNull(feed.commission_fee, "feed: commission_fee set");
      assert.approximately(
        feed.commission_fee,
        SELF_FEE,
        0.01,
  ...
  test.concurrent("declined → accepted with fee and amount change", ({ ctx }) =>
    ctx.track_bg_rejections(async () => {
      ...
      assert.isNotNull(updated_feed.commission_fee, "feed: commission_fee set");
      assert.approximately(
