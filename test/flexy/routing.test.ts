...
import type { ProcessingUrlResponse } from "@/entities/payment";
    ...
    test.concurrent("Basic routing", async ({ ctx }) => {
      await ctx.track_bg_rejections(async () => {
        ...
        let millennium = ctx.mock_server(
          MillenniumPayment.mock_params(ctx.uuid),
        );
...
async function executeRoutingChain(
  ...
  currency: string,
  gateways: Routable[],
  onProccessingUrlResponse?: (processingUrl: ProcessingUrlResponse) => {},
) {
  let merchant = await ctx.create_random_merchant();
  ...
  onProccessingUrlResponse?.(await processingUrl);
