...
import {
  type TraderSchemas,
  throwResponseErrors,
} from "@/driver/trader/traderFetchClient";

export type ExtendedRequisite = ReturnType<typeof extendedRequisite>;

async function edit(
  this: ExtendedRequisite,
  update: Partial<TraderSchemas["requisite"]>,
) {
  await delay(1000);
  ...
  assert(
    requisite,
    `Requisite ${this.id} for profile ${this.profile_id} was not found`,
  );
...
export function extendedRequisite(
  driver: TraderDriver,
  id: number,
  profile_id: number,
) {
